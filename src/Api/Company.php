...
/*
 * Pi::api('company', 'video')->getVideoList($params);
 * Pi::api('company', 'video')->getVideoIdList($params);
 * Pi::api('company', 'video')->getOrders($params);
 */
...
class Company extends AbstractApi
{
    public function getVideoList($params): array
    {
        ...
        foreach ($rowSet as $row) {
            $list[$row->id] = Pi::api('video', 'video')->canonizeVideoLight($row);
        }

        return $list;
    }

    public function getVideoIdList($params): array
    {
        // Set info
        $where   = ['company_id' => $params['company_id']];
        $order   = ['time_create DESC', 'id DESC'];
        ...
        // Select
        $select = Pi::model('video', $this->getModule())->select()->columns($columns)->where($where)->order($order);
        $rowSet = Pi::model('video', $this->getModule())->selectWith($select);
    ...
    {
        // Set info
        $where   = ['module' => $this->getModule(), 'product_type' => 'video', 'product' => $params['videoList']];
        $order   = ['id DESC'];
        $columns = ['order'];
